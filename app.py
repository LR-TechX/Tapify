...
WALK_UPGRADES = {
    ...
    2: {"rate": Decimal("0.02"), "price": Decimal("500.00")},
    3: {"rate": Decimal("0.05"), "price": Decimal("1500.00")},
    4: {"rate": Decimal("0.10"), "price": Decimal("4000.00")},
}

# Aviator parameters
AVIATOR_GROWTH_PER_SEC = Decimal("0.25")
MIN_BET = Decimal("1000.00")
MAX_BET = Decimal("1000000.00")

# Wallet params
MIN_DEPOSIT = Decimal("1000.00")
MIN_WITHDRAW = Decimal("50000.00")
...
def get_or_create_user_from_query():
    ...
    if not user:
        ...
        # Insert signup bonus as transaction
        bonus = Decimal("8.00")
        db.session.add(
            Tx(chat_id=user.chat_id, type="signup_bonus", status="approved",
               amount_usd=bonus, meta={"ngn": str(to_cents(bonus * USD_TO_NGN))})
        )
        db.session.commit()

        # Sync to make sure balances match transactions
        sync_user_balance(user)
    else:
        if user.ensure_defaults():
            db.session.commit()
        # 🔑 Always recalc balance from transactions
        sync_user_balance(user)
...
def api_user():
    ...
    # balances are always synced at this point
    return jsonify({
